...
export const outboxSchema = new mongoose.Schema(
	{
		...
		// Timeout tracking for saga compensation
		expiresAt: {
			type: Date,
			index: true,
			description: 'Expiration timestamp for saga timeout (triggers compensation)',
		},

		// Compensation data snapshot
		compensationData: {
			type: mongoose.Schema.Types.Mixed,
			description: 'Snapshot of data needed for compensation/rollback',
		},
...
outboxSchema.index({ status: 1, expiresAt: 1 }) // For timeout worker queries
