...
export class OutboxProcessor {
	...
	async start() {
		...
		try {
			...
			logger.info(
				{ serviceName: this.serviceName },
				'✓ Outbox Processor started, watching for events'
			)
		} catch (error) {
			logger.error(
				{
	...
	async _reconnectChangeStream() {
		try {
			...
			logger.info(
				{ serviceName: this.serviceName },
				'✓ Change stream reconnected successfully'
			)
		} catch (error) {
			logger.error(
				{
			...
			if (this.isRunning) {
				logger.info(
					{ serviceName: this.serviceName },
					'⏳ Retrying reconnection in 10 seconds...'
				)
				setTimeout(async () => {
					await this._reconnectChangeStream()
				}, 10000)
