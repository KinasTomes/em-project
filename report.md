...
| Chỉ số                  | Trước | Sau           |
| ----------------------- | ----- | ------------- |
| Thời gian tạo đơn       | 3–5s  | **250–400ms** |
| % đơn bị pending quá 3s | 12%   | **1.5%**      |
| Tỷ lệ duplicate đơn     | 3.2%  | **0.7%**      |

### 🧪 Testing

- Load test 500 RPS với k6 trong 3 phút
- Kiểm tra độ sâu hàng đợi + throughput publish RabbitMQ
- Measure response time TTFB
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  │                PENDING → CANCELLED (inventory fail)            │
  │                CONFIRMED → CANCELLED (payment fail)            │
  └─────────────────────────────────────────────────────────────────┘

  COMPENSATION FLOW (khi Payment fail):
  ┌─────────┐    PAYMENT_FAILED     ┌───────────┐    RELEASE_STOCK
  │ Payment │ ─────────────────────▶│ Inventory │ ─────────────────▶ +stock
  │ Service │                       │  Service  │
  └─────────┘                       └───────────┘
```
...
| Chỉ số                                         | Trước | Sau Saga |
| ---------------------------------------------- | ----- | -------- |
| Tỷ lệ order stuck (PENDING > 2 phút)           | 8.4%  | **0.4%** |
| Tỷ lệ thiếu hàng ảo                            | 3.1%  | **0.2%** |
| Tỷ lệ thanh toán retry thất bại không rollback | 6%    | **<1%**  |

### 🧪 Testing

- Chaos testing: random kill Inventory Service
- Simulate payment gateway timeout 30%
- Measure số lượng events compensation
