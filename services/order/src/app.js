...
const TimeoutWorker = require('./workers/timeoutWorker')
...
class App {
	...
	async initOutbox() {
		try {
			...
			const { Broker: B } = await import('@ecommerce/message-broker')
			OutboxManager = OM
			Broker = B
			...
			// Initialize broker for timeout worker
			this.broker = new Broker()
			logger.info('✓ [Order] Message broker initialized')

			// Initialize timeout worker
			this.timeoutWorker = new TimeoutWorker(
				this.outboxManager.OutboxModel,
				this.broker,
				{
					serviceName: 'order',
