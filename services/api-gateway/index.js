...
app.use("/auth", (req, res) => {
  logger.info({ path: req.path, method: req.method }, "Routing to auth service");
  proxy.web(req, res, { target: config.authServiceUrl });
});

// Route requests to the product service
app.use("/products", (req, res) => {
  logger.info({ path: req.path, method: req.method }, "Routing to product service");
  proxy.web(req, res, { target: config.productServiceUrl });
});

// Route requests to the order service
app.use("/orders", (req, res) => {
  logger.info({ path: req.path, method: req.method }, "Routing to order service");
