...
class InventoryService {
  ...
  /**
   * Reserve inventory for an order (with Outbox Pattern)
  ...
  async reserveStock(productId, quantity, orderId) {
    ...
    try {
      // Use an atomic reserve operation within transaction
      ...
      if (!updated) {
        ...
        // Create RESERVE_FAILED event (outside transaction)
        await OutboxRepository.create({
          eventType: 'INVENTORY_RESERVE_FAILED',
          aggregateId: orderId || productId,
          aggregateType: 'Order',
          payload: {
            orderId,
            productId,
            quantity,
            availableStock: current ? current.available : 0,
        ...
        return {
          success: false,
          message: `Insufficient stock. Available: ${current ? current.available : 0}, Requested: ${quantity}`,
          inventory: current,
        };
      }

      // Create RESERVED event in outbox (within same transaction)
      await OutboxRepository.create({
      ...
      // Commit transaction - both DB update and event insert are atomic
      await session.commitTransaction();

      logger.info(
        `[InventoryService] Reserved ${quantity} units for product ${productId} (Order: ${orderId})`
      );
  ...
  /**
   * Release reserved inventory (with Outbox Pattern)
   */
  async releaseReserved(productId, quantity, orderId) {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const updated = await inventoryRepository.updateByProductId(
        productId, 
        {
          $inc: {
            available: quantity,
      ...
      // Create RELEASED event in outbox
      await OutboxRepository.create({
      ...
      logger.info(
        `[InventoryService] Released ${quantity} reserved units for product ${productId} (Order: ${orderId})`
      );
      return updated;
    } catch (error) {
      await session.abortTransaction();
  ...
  /**
   * Restock inventory (with Outbox Pattern)
   */
  async restockInventory(productId, quantity) {
    ...
    try {
      ...
      // Create RESTOCKED event in outbox
      await OutboxRepository.create({
  ...
  /**
   * Adjust inventory with Outbox Pattern (manual adjustment by admin)
   */
  async adjustInventory(productId, availableDelta, reservedDelta = 0) {
    ...
    try {
      ...
      // Create STOCK_UPDATED event in outbox
      await OutboxRepository.create({
        ...
        payload: {
          ...
          oldReserved: inventory.reserved,
