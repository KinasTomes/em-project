...
const localMessageBroker = require('./src/utils/messageBroker')
...
async function startServer() {
	try {
		...
		// Connect local message broker (for compensation handlers)
		logger.info('Connecting local message broker...')
		await localMessageBroker.connect()
		logger.info('✓ [Inventory] Local message broker connected and consuming')
...
process.on('SIGTERM', async () => {
	...
	if (localMessageBroker.connection) {
		await localMessageBroker.connection.close()
	}
	if (broker) {
		await broker.close()
	}
